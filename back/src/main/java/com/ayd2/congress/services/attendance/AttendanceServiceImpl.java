...
public class AttendanceServiceImpl implements AttendanceService {
    ...
    public AttendanceServiceImpl(ActivityService activityService, AttendanceRepository attendanceRepository,
            ...
            CongressService congressService, ActivityMapper mapper) {
    ...
    @Override
    public void createAttendance(NewAttendanceRequest request)
            throws NotFoundException, DuplicatedEntityException, ActivityFullException, ActivityAlreadyEndendException,
            ActivityNotStartedException, CongressNotStartedException {
        ...
        boolean exists = attendanceRepository.existsByActivityIdAndUserIdAndType(request.getActivityId(), user.getId(),
                request.getType());
        if (exists)
            throw new DuplicatedEntityException("Attendance already exists");
        boolean isInscribed = inscriptionService.isUserEnrolledInCongress(user.getId(), congress.getId());
        if (!isInscribed)
            throw new NotFoundException("User is not inscribed in the congress");
        ...
        if (acitivty.getType() == ActivityType.WORKSHOP && request.getType() == AttendanceType.ATTENDANCE) {
            ...
            if (!isEnrolled)
                throw new NotFoundException("User is not enrolled in the workshop");
        } else if (acitivty.getType() == ActivityType.WORKSHOP
                && request.getType() == AttendanceType.WORKSHOPINSCRIPTION)
            verifyCapacity(acitivty.getId());

        if (request.getType() != AttendanceType.WORKSHOPINSCRIPTION) {
            if (congress.getStartDate().isAfter(request.getDate()))
                throw new CongressNotStartedException("Congress has not started yet");
            if (acitivty.getStartDate().isAfter(request.getDate()))
                throw new ActivityNotStartedException("Activity has not started yet");
        }
        if (acitivty.getEndDate().isBefore(request.getDate()))
            throw new ActivityAlreadyEndendException("Activity has already ended");
    ...
    public boolean verifyCapacity(Long activityId)
        ...
        if (activity.getCapacity() == 0)
            throw new ActivityFullException("Activity has no capacity");
        Long currentAttendanceCount = attendanceRepository.countByActivityIdAndType(activityId,
                AttendanceType.WORKSHOPINSCRIPTION);
        if (currentAttendanceCount >= activity.getCapacity())
    ...
    public List<AttendanceResponse> getAttendanceByUserId(Long id) throws NotFoundException {
        ...
        if (attendances.isEmpty())
            throw new NotFoundException("No attendance found for the user");
    ...
    public List<AttendanceResponse> getAttendanceByActivityId(Long id) throws NotFoundException {
        List<AttendanceEntity> attendances = attendanceRepository.findByActivityIdAndType(id,
                AttendanceType.ATTENDANCE);
        if (attendances.isEmpty())
            throw new NotFoundException("No attendance found for the activity");
        return attendanceMapper.toResponseList(attendances);
    }
    ...
    public boolean isEnrolledInWorkshop(Long userId, Long activityId) throws NotFoundException {
        ...
        if (activity.getType() != ActivityType.WORKSHOP)
            throw new NotFoundException("Activity is not a workshop");
        return attendanceRepository.existsByActivityIdAndUserIdAndType(activityId, userId,
                AttendanceType.WORKSHOPINSCRIPTION);
    }

    @Override
    public List<AttendanceEntity> getAttendanceByUserIdAndCongressId(Long userId, Long congressId, AttendanceType type)
            throws NotFoundException {
        userService.getById(userId);
        congressService.getById(congressId);
        return attendanceRepository.findAllByUserIdAndActivityCongressIdAndType(userId, congressId,
                type);

    }

    @Override
    public List<ActivityResponse> getByUserIdAndCongressId(Long userId, Long congressId) throws NotFoundException {
        List<AttendanceEntity> entities = getAttendanceByUserIdAndCongressId(
                userId, congressId, AttendanceType.WORKSHOPINSCRIPTION);

        return activityMapper.toActivityResponseList(
                entities.stream()
                        .map(AttendanceEntity::getActivity)
                        .distinct()
                        .toList());
    }
